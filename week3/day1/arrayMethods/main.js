const people= [
  ...
  const emailCompany= people.map(p => {return {email: p.email, companyName: p.company.name}})
  console.log(emailCompany)

  const zipcode= people.filter(p =>p.address.zipcode[0]==='5')
  console.log(zipcode)

  const names= people.map(p => p.name).filter(n => n[0]==="C")
  console.log(names);

  console.log(people.every(p => p.address.city==="South Christy"))

  const user= people.find(p =>p.address.suite==="Apt. 950")
...
//     const toLowerCase= messagesFromDad.map(m => m.toLowerCase())
...
//         let tempPost= posts.find(p=> p.id===postID)
//         let comment= tempPost.comments.find(c=> c.id===commentID)
//         console.log(comment)
...
//     function byMarvel (){
//         console.log(movies.some(m => m.studio==='Marvel'))
//     }
//     byMarvel();